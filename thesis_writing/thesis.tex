...
\hline
Mode & Level & Pairing Security & Message Security & Since Version \\ \hline
1 & 1 & No pairing & No security & $\geq$ 4.0 \\ \hline
1 & 2 & Unauthenticated legacy & Encryption + MAC & $\geq$ 4.0 \\ \hline
1 & 3 & Authenticated legacy & Encryption + MAC & $\geq$ 4.0 \\ \hline
1 & 4 & Authentication LE-SC & Encryption + MAC & $\geq$ 4.2 \\ \hline
2 & 1 & Unauthenticated & GATT layer data signing & $\geq$ 4.0 \\ \hline
2 & 2 & Authenticated & GATT layer data signing & $\geq$ 4.0 \\ \hline
3 & 1 & No pairing & No security & $\geq$ 5.2 \\ \hline
3 & 2 & Unauthenticated & Encryption + MAC & $\geq$ 5.2 \\ \hline
3 & 3 & Authenticated & Encryption + MAC & $\geq$ 5.2 \\ \hline
\end{tabular}
\caption{Overview of BLE security modes and levels \cite{caesar2022survey}}
\label{table:ble-security}
\end{table}
...
Pairing procedures involve the agreement of keys between the devices involved in the connection, allowing for encrypted data exchange. This procedure is required for all security modes and levels, as shown in Table \ref{table:ble-security}, except for mode 1 level 1 and mode 3 level 1, which provide no security features at all.

% \begin{figure}[h!]
% \centering
% \includegraphics[width=\textwidth]{pairing-procedure}
% \caption{Pairing procedure \cite{caesar2022survey}}
% \label{fig:pairing}
% \end{figure}

As shown in Figure \ref{fig:pairing}, the pairing procedure consists of two different phases. The first phase involves the exchange of security features, such as association modes, whether MITM protection is required, and the input-output capabilities of both devices to reach an agreement on the security level and mode. Additionally, during this phase, devices exchange the possible bonding request.

The second phase involves the actual exchange of keys, which will be used to encrypt future connections. Depending on the information exchanged in phase 1, the mode of pairing is chosen. Various methods include "Just Works", "Passkey Entry", "Numeric Comparison", and "Out of Band". Each method varies in the level of security provided.

% \begin{figure}[h!]
% \centering
% \includegraphics[width=\textwidth]{pairing-authentication-method}
% \caption{Pairing authentication method based on the input/output capabilities \cite{nordic2021}}
% \label{fig:pairing-auth}
% \end{figure}

\section{Bonding}

The pairing procedure could involve a further phase, phase 3, if bonding is required. Bonding allows devices to avoid restarting the pairing process each time there is a new connection, useful for devices that enter standby mode and need to restore communication. Phase 3 involves exchanging long-term keys (LTK) and optional keys such as the Identity Resolving Key (IRK) and the Connection Signature Resolving Key (CSRK).

% \begin{figure}[h!]
% \centering
% \includegraphics[width=\textwidth]{bonding-procedure}
% \caption{Summary of the pairing procedure with all three phases \cite{nordic2021}}
% \label{fig:bonding}
% \end{figure}

\section{Legacy Pairing and Low Energy Secure Connection (LE-SC)}

Since the introduction of Bluetooth Low Energy version 4.2, a further level in mode 1 has been added: Low Energy Secure Connection (LE-SC). LE-SC uses elliptic curve Diffie-Hellman (ECDH) instead of the Short-Term Key (STK), which allows devices to choose a private key without sharing it over the air, thereby providing enhanced security.

% \begin{figure}[h!]
% \centering
% \includegraphics[width=\textwidth]{LE-SC-pairing-procedure}
% \caption{LE-SC pairing procedure \cite{caesar2022survey}}
% \label{fig:le-sc}
% \end{figure}

LE-SC supports association modes like "Just Works", "Numeric Comparison", "Passkey Entry", and "Out of Band", providing different levels of security.

\section{Encryption}

Once the pairing procedure ends, the generated keys (LTK or STK) are used to encrypt the connection. Figure \ref{fig:encryption} shows the encryption procedure in BLE.

% \begin{figure}[h!]
% \centering
% \includegraphics[width=\textwidth]{encryption-procedure}
% \caption{Encryption procedure in BLE \cite{caesar2022survey}}
% \label{fig:encryption}
% \end{figure}

\section{Man-in-the-Middle Attack}

MITM attacks involve a hacker intercepting and potentially altering communications between two devices. BLE security features can protect against MITM attacks if both legitimate devices require MITM protection and have input/output capabilities.

\section{Modes 2 and 3}

Modes 2 and 3 provide different security features in Bluetooth Low Energy. Mode 2, based on connection-based data signing, ensures data authenticity and reliability. Mode 3 is used for broadcast connections where encryption is required.

\begin{thebibliography}{9}

\bibitem{caesar2022survey}
Matthias Cäsar et al., "A survey on Bluetooth Low Energy security and privacy", Elsevier, 06/01/2022. DOI: \href{https://doi.org/10.1016/j.comnet.2021.108712}{10.1016/j.comnet.2021.108712}.

\bibitem{circuits2021}
\href{https://www.allaboutcircuits.com/technical-articles/bluetooth-le-security-modes-and-procedures-explained/}{All About Circuits: Bluetooth LE Security Modes and Procedures Explained}.

\bibitem{nordic2021}
\href{https://academy.nordicsemi.com/courses/bluetooth-low-energy-fundamentals/lessons/lesson-5-bluetooth-le-security-fundamentals/topic/pairing-process/}{Nordic Semiconductor Academy: Bluetooth LE Security Fundamentals}.
...
Even if in previous firmware implementation the data that were exchanged from one board to the other one were directly generated from the peripheral itself, this would not be the case in the actual implementation, in which data would directly come from the ADC, whose samples will contain information which are relative to the neural system. This sampling procedure is done at a frequency of 80 kHz. Each sample will then be provided to the peripheral board, which is the one responsible to send them to the central device and then to the external unit. All of these data which come from the ADC have a size of 10 bits, which is quite uncomfortable to be handled. The microcontroller, in fact, needs to store the samples, but storing them into a \texttt{uint8\_t} variable would imply a waste of information, variables of 16 bits size would instead guarantee a waste of memory. In the previous implementation, the second approach is adopted, as shown below:
\begin{center}
\begin{tabular}{|c|c|}
    \hline
    16 bits variable & \\
...
As mentioned above, this approach is responsible for a great waste of memory. In fact, every time a new 10-bit sample is generated from the ADC, this is saved into 16-bit variables, guaranteeing for each sample a waste of 6 bits. This approach should be improved because memory in microcontrollers is very limited, and 6 bits out of 16 wasted for each sample means that $\frac{6}{16}$ of the throughput obtained is not informative. In all of the online tests two different throughputs are computed as explained below:

\[
\text{Throughput} = \frac{\text{n° bits received from central device}}{\text{elapsed time}}
\]

\[
\text{Real throughput} = \frac{\text{n° bits received from central device} - \text{n° empty bits}}{\text{elapsed time}}
\]

Where the first one represents the effective speed of the BLE communication, while the second one takes into account just the number of bits which contain information, which would result more or less in $\frac{10}{16}$ of the first one. In order to have a better understanding of what was happening with the previous version of the firmware, the pre-processing operations done for each single BLE packet could be analyzed. The function responsible for doing this is the \texttt{Spibuff} function. As previously said, data were generated at a constant frequency of 80 kHz. Once there are enough samples inside the ring buffer to fulfill a BLE packet (which means at least 244 bytes), data preprocessing operations were performed. Since each sample has a size which cannot be contained into a single byte, this means 2 bytes are dedicated to each one, and then data preprocessing operation is performed each time 122 samples have been generated from ADC. Once they are, each sample is taken from the ring buffer. Then, the 8 most informative bits of the \texttt{uint16\_t} variable which temporarily contains it would form the first byte, and the 8 less informative bits would form the second one. This operation is performed 122 times for each BLE packet, in order to fulfill them. By doing this, the final number of samples received from the central device is half of the received bytes. The figure below represents this concept in case of an offline test.

Bytes generation procedure done in the previous implementation is schematized below, where the parentheses represent the bytes that would be contained in the BLE packet and sent to the central device:
\begin{center}
\begin{tabular}{|c|c|c|}
    \hline
    16 bits variable & 1\textsuperscript{st} byte & 2\textsuperscript{nd} byte \\
    \hline
\end{tabular}
\end{center}

Each couple of bytes would then be packed into the original 16 bits variable in the central device, in order to reconstruct the ADC samples. To improve this operation, a shifting algorithm has been implemented. The idea is to perform some bit-masking operations in order to use also those bits which previously were empty. As a consequence, data need to be compressed in the peripheral and then decompressed in the central device. A scheme of the compression operation is shown below:

To be precise the ADC samples are saved into \texttt{uint16\_t} variables, but since they have a size of 10 bits just the 10 less significant bits out of this 16 are considered. The shifting procedure performed by the peripheral can be summarized in the following steps:
\begin{enumerate}
    \item First byte is obtained by taking the 8 most significant bits of the first sample obtained from the ADC
    \item Second byte is obtained by putting the 2 less significant bits of the first sample into the 2 most significant positions and the 6 most significant bits of the second sample into the remaining 6 positions
    \item Third byte is obtained by putting together the four right positioned bits of the second sample and the 4 left positioned bits of the third sample
    \item Fourth byte is obtained by considering the 6 less significant bits of the third sample and the 2 most significant bits of the fifth sample
    \item Lastly, the fifth byte is obtained by taking the 8 most rightly positioned bits inside the fourth sample
\end{enumerate}

Once these steps are completed, the procedure becomes periodical, and it is repeated from the beginning. By doing this, it can be noted that, in order to send 4 samples, just 5 bytes are necessary and not 8 like in the previous implementation. Moreover, each single bit contains information, and it is not wasted. However, it should also be noted that this procedure can only be performed when at least 4 samples have been generated from the ADC, differently from before where just one was necessary. Another limitation is imposed by the BLE packet size, which can contain 244 bytes as a maximum value, which is not a number multiple of five. This means that not all of the samples inside a single BLE packet can be shifted if the BLE maximum packet size is maintained.

To be more precise, the generation of a single BLE packet can be analyzed as done for the previous implementation. As previously said, shifting operations can be performed if the number of samples is more than four and the number of bytes generated does not exceed the maximum size of 244 bytes. Then, these conditions imply that a single BLE packet of 244 bytes is composed of 240 bytes which contain the samples shifted. This number is the highest multiple of five which does not exceed the limit of 244 bytes. The number of samples that can be contained into these 240 bytes can be computed by considering that 5 bytes contain 4 samples as previously explained. As a consequence, the exact number of samples could be computed by multiplying 240 bytes times $\frac{4}{5}$, which would result in 192 samples. This concept is summarized in the following formula:

\[
\text{n° shifted samples} = \text{n° shifted bytes} \times \frac{4}{5}
\]

Once this procedure is completed, however, the BLE maximum size has not been reached yet. In fact, since only 240 bytes are generated, there is still the possibility to send other two samples by organizing them in four bytes, as done in the previous implementation. In this way the maximum size of 244 bytes is reached, where 240 bytes contain the samples shifted and the last 4 contain the samples organized as in the previous implementation. This means that, in a single BLE packet whose size is 244 bytes, 194 samples can be contained.

\[
\text{n° samples in a BLE packet} = \text{n° shifted samples} + \frac{\text{n° remaining bytes}}{2}
\]

It should also be noted that, by making a comparison between the previous implementation and the new one with the shift algorithm, the number of empty bits is way different. In a full-sized BLE packet which was packed with the previous implementation procedure, there were 6 empty bits for each 16 bits variable which contained the 122 samples, resulting in 732 empty bits for each BLE packet of 244 bytes size. With the shift algorithm, just the last 2 samples are not packed optimally, implying that the empty bits are just 12.

Another comparison can be made considering an offline test composed of 32400 bytes. In fact, in order to have a better understanding of what this decreasing of empty bits would imply, it could be asked what is the number of samples that will be received by the central device at the end of a test. By taking into account an offline test, the total number of BLE packets shared during the whole communication should firstly be computed by dividing the size of the communication by the size of each packet, resulting in 133 packets exchanged, where the last one would not be fully filled.

\[
\text{n° BLE packets} \approx \frac{\text{n° bytes received}}{\text{maximum BLE packet size}}
\]

The size of the last packet can be computed by subtracting from the 32400 bytes which form the whole offline test, the number of bytes received through all of the BLE packets except for the last one. Specifically, this would mean that the whole offline test of 32400 bytes is composed of 133 BLE packets, but 32400 minus 324 * 132, resulting in the last BLE packet size being equal to 168 bytes. By taking these pieces of information, the number of samples that would be received through the whole communication with the shifting procedure can be computed. To be precise, the whole offline test would then be composed of 132 BLE packets whose size is 244 bytes and 1 packet of 168 bytes, so the number of samples received could be computed by taking the sum of the product between 192 samples and 132 and the product between 4 and 32.

\[
\text{n° samples received} = (\text{n° samples in 244 bytes packet} \times \text{n° full-sized BLE packets}) + (\text{remaining bytes} \times \frac{4}{5})
\]

By performing this operation, it would result in the whole communication that the central device would receive in 25472 samples. By comparing this with the previous implementation, the number of samples that would be received should be computed in the same way. Specifically, in order to understand the real advantage obtained with this new shifting algorithm, the number of samples in a full-sized packet of 244 bytes should be computed as done previously, by considering that in the previous implementation 2 bytes contained a single sample, resulting in 122 samples in a single packet. As done previously, the last packet whose size is 168 bytes should be considered as well, and the number of samples would be then given by the integer part of 168 divided by 2, which would result in 84 samples in the last packet. This means that the total number of samples that are received in the previous implementation is given by 132 multiplied by 122 plus 84, and this would result in a total number of 16248 samples received, which is lower than the one obtained with the new shift algorithm. The improvement can then be computed by taking the ratio between 16248 and 25472, resulting in a percentage which would equal approximately 35%.
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{peer_manager_architecture.png}
%     \caption{Peer Manager architecture\footnote{\url{https://infocenter.nordicsemi.com/index.jsp?topic=\%2Fsdk_nrf5_v17.1.0\%2Flib_peer_manager.html}}}
%     \label{fig:peer_manager_architecture}
% \end{figure}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{connection_interval_timeout.png}
%     \caption{Connection interval and connection supervision timeout\footnote{\url{https://docs.silabs.com/bluetooth/latest/bluetooth-fundamentals-connections/multi-peripheral-topology}}}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{sniffed_connection_parameter_update.png}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{connection_parameters_wireshark.png}
%     \caption{Connection parameters in Wireshark}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{ideal_tests_without_security.png}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{ideal_tests_with_security.png}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{multicentral_implementation.png}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{direct_communication_board_PC.png}
%     \caption{Direct communication board-PC}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{ppk2_source_amperemeter_mode.png}
%     \caption{PPK2 in source and ammeter mode}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{pin_connections_source_meter.png}
%     \caption{Pin connections in Source Meter mode\footnote{\url{https://infocenter.nordicsemi.com/index.jsp?topic=%2Fug_ppk2%2FUG%2Fppk%2Fsetting_up.html}}}
%     \label{fig:pin_connections_source_meter}
% \end{figure}

% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{pin_connections_amperemeter.png}
%     \caption{Pin connections in Ammeter mode\footnote{\url{https://infocenter.nordicsemi.com/index.jsp?topic=%2Fug_ppk2%2FUG%2Fppk%2Fsetting_up.html}}}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{power_profiler_kit_app.png}
%     \caption{Power profiler kit app}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{power_profiler_app_source_meter_mode.png}
%     \caption{Power profiler app source meter mode}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{power_profiler_app_amperemeter_mode.png}
%     \caption{Power profiler app ammeter mode}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{example_measurement_ppk2.png}
%     \caption{Example of measurement done with PPK2}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{SB9_solder_bridge.png}
%     \caption{SB9 solder bridge}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{SB12_solder_bridge.png}
%     \caption{SB12 solder bridge}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{cutting_process_SB9.png}
%     \caption{Cutting process of SB9}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{current_measurement_BLE_communication.png}
%     \caption{Current measurement during a BLE communication}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{current_consumption_connection_interval.png}
%     \caption{Current consumption connection interval}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{boards_distance_10cm.png}
%     \caption{Boards at a distance of 10cm}
%     \label{fig:boards_distance_10cm}
% \end{figure}

% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{boards_distance_50cm.png}
%     \caption{Boards at a distance of 50cm}
%     \label{fig:boards_distance_50cm}
% \end{figure}

\section*{Summary}
\begin{enumerate}
    \item Multicentral implementation \dotfill 2
    \item Receiver firmware multicentral \dotfill 3
    \item Transmitter firmware multicentral \dotfill 5
    \item Command sending process \dotfill 6
\end{enumerate}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{previous_implementation_command_sending.png}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{memory_allocation_seggar.png}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{memory_allocation_error.png}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{faulty_error_state_central_board.png}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{nrf_connect_scanning.png}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{nrf_connect_connected.png}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{nrf_connect_advertisements.png}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{nrf_connect_commands.png}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{nrf_connect_send_data.png}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{nrf_connect_services.png}
%     \caption{nRF Connect services menu}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{nrf_connect_log.png}
%     \caption{nRF Connect log menu}
%     \label{fig:nrf_connect_log}
% \end{figure}

\section{Direct Communication between Board and Windows PC}
An alternative approach to the previous method involving two boards and UART communication is to have the peripheral board directly communicate with the external unit through the PC's Bluetooth.

In this alternative, the peripheral board acts as a transmitter, while the PC serves as the central device that receives data. Tools such as Sniffer and Wireshark are used to monitor the data exchange and connection process, although they cannot scan retransmissions. The Matlab BLE library \cite{matlab_ble} is employed for this purpose. This library enables developers to connect to a BLE device, subscribe and unsubscribe to specific characteristics, and view nearby BLE devices that are advertising. Despite not being optimized for Nordic boards, this library allows for communication in which the PC receives data. The code used by the board to act as a peripheral device remains the same as in the security implementation. It was noted that the previous firmware implementation did not work with the Matlab BLE library. Therefore, only the central device code will be illustrated in this section.

\subsection{Offline Receiver Code}
The central device code is written in Matlab and is simpler compared to the board-to-board approach. Matlab provides a list of nearby Bluetooth devices through the function \texttt{blelist}. When this function is executed, the user can view something similar to Figure \ref{fig:matlab_blelist} in the command window.

% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{matlab_blelist.png}
%     \caption{Matlab \texttt{blelist}}
%     \label{fig:matlab_blelist}
% \end{figure}

The \texttt{blelist} function displays the number of devices advertising via BLE, their names (if known), addresses, advertising power in dB, and the "Advertisement" field, which contains information about the advertising device, as shown in Figure \ref{fig:matlab_advertisement}.

% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{matlab_advertisement.png}
%     \caption{Advertisement field in Matlab}
%     \label{fig:matlab_advertisement}
% \end{figure}

The next step in the Matlab code involves creating a connection between the board and PC using the \texttt{ble} function. Once the connection is established, a new object is created, containing the following fields, as shown in Figure \ref{fig:matlab_ble_object}:

% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{matlab_ble_object.png}
%     \caption{BLE object in Matlab}
%     \label{fig:matlab_ble_object}
% \end{figure}

This object includes the device name, address, a Boolean variable indicating whether the device is connected to the PC, and the device's services and characteristics. These characteristics are crucial because the developer needs to subscribe to them to receive and transmit data. The relevant characteristic fields are illustrated in Figure \ref{fig:matlab_characteristics}.

% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{matlab_characteristics.png}
%     \caption{Characteristics of Senseback BLE object in Matlab}
%     \label{fig:matlab_characteristics}
% \end{figure}

The last two characteristics shown in Figure \ref{fig:matlab_characteristics} are of particular interest. The first allows the developer to write data to the board, essential for sending commands, while the second is used to receive data from the board, necessary for receiving BLE packets from the peripheral. The \texttt{subscribe} function is used to subscribe to a specific characteristic, which, in this application, is required only for the last characteristic.

Once the device is connected and Matlab has subscribed to the characteristic, global variables are defined. Unlike C language, Matlab requires global variables to be defined each time a new function is entered. The user then inserts the command, which, in the offline case, is done only once. Before sending, each character in the string is converted to 8-bit values and will be converted back to a string once received by the peripheral. Commands are sent in write without response mode.

The final step in the main code is calling the function that receives the data, done through a callback function, which is a function automatically executed in response to a specific event \cite{matlab_callbacks}. In this application, the callback is triggered each time a new BLE packet is received from the peripheral. This is particularly useful for online code because the function continuously works while the command window waits for the STOP command.

The callback function is dedicated to receiving data. It first re-defines all global variables used in the main code. It checks if the function is triggered for the first time. If so, a timer is triggered through the \texttt{tic} function to count the elapsed time for the communication. The Boolean variable is set to false to prevent multiple timer starts. The first packet is read using the \texttt{read} function, which reads data from a characteristic. The last two bytes of each received BLE packet are stored, and a byte counter is incremented. A matrix with 244 columns and rows equal to the number of received BLE packets is created. Each new packet creates a new row, with the last packet's columns filled with zeros to reach 244 values if needed. After the online test, the vector looks like Figure \ref{fig:matlab_received_data}.

% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\textwidth]{matlab_received_data.png}
%     \caption{Received data from board in Matlab}
%     \label{fig:matlab_received_data}
% \end{figure}

As shown, bytes received from the board without shift are gradually increased. If the size of the last received packet is at least two bytes and both are equal to 255, the data receiving process ends. The central device then unsubscribes from the characteristic using the \texttt{unsubscribe} function, stops the timer, and computes the throughput by dividing the total number of bits by the elapsed time. Unlike C, Matlab timers use seconds as units, so the result must be multiplied by 1000 to convert to milliseconds.

\subsection{Online Receiver Code}
The online receiver code is the same as the offline case, with the only difference being the number of commands required, which is two in this case instead of one. This is possible due to the callback function. Without it, Matlab executes commands sequentially, blocking the receiving process until both commands are entered, which is not the desired behavior.

\subsection{Receiver Code with Shift}
The process of receiving data with and without shift is similar; the difference lies in how the data is handled after receipt. When shift is not involved, data are immediately readable. When the peripheral has shifted data, it needs to be decompressed to be readable. Figure \ref{fig:matlab_shifted_bytes} shows how shifted data looks before decompression, compared to Figure \ref{fig:matlab_characteristics}.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{matlab_shifted_bytes.png}
    \caption{Shifted bytes received from Bluetooth PC}
    \label{fig:matlab_shifted_bytes}
\end{figure}

Data shifting is performed similarly to the receiver code of the shift algorithm, with the difference being the programming language. In Matlab, bit-shifting operations are performed using \texttt{bitor} and \texttt{bitshift} functions \cite{matlab_bitor, matlab_bitshift}, which perform bitwise operations and bit shifting, respectively. Once decompressed, the data are saved in a one-dimensional vector, as shown in Figure \ref{fig:matlab_shifted_data}.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{matlab_shifted_data.png}
    \caption{Shifted data received by PC}
    \label{fig:matlab_shifted_data}
\end{figure}

A major advantage of this approach is that data decompression can be performed without RAM limitations. Since the operation occurs after all data have been received, it does not affect the speed. Future developments could include measuring the time required to pack and unpack the data for both board-to-board and board-to-PC cases.

\subsection{Reverse Transmission}
Previous setups involved a peripheral board sending data to a central device, which could be a PC, another board, or a smartphone. However, in the current Senseback project, data should be transmitted both outside the body and to stimulate nerves, requiring bi-directional communication.

Initially, the idea is to have the two devices exchange roles every 32,400 bytes, as in an offline test, until the user enters the STOP command. Each exchange computes a throughput. Modifications were made to both the board and external unit codes.

\subsubsection{Board Side}
The process starts when the user enters the START command. The board sends 32,400 bytes to the PC and then prepares to receive data from the PC. The \texttt{nus\_data\_handler} function, previously used only for receiving commands from the central device, now handles data reception. A new timer, \texttt{timer3}, is added to compute the elapsed time for each data receiving session. To enable this timer, \texttt{NRFX\_TIMER3\_ENABLED} and \texttt{TIMER3\_ENABLED} in the configuration file must be set to true. The functioning of the three timers is summarized below.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{timer_summary.png}
    \caption{Timers summary for reverse transmission}
    \label{fig:timer_summary}
\end{figure}

An example of reverse transmission communication is as follows:
\begin{enumerate}
    \item Command START is received from the central device.
    \item Data generation starts, and timer2 is enabled to count elapsed time during data sending.
    \item All 32,400 bytes are sent.
    \item Timers timer1 and timer2 are disabled.
    \item The central device starts sending data.
    \item Upon receiving the first data, timer3 is enabled.
    \item After all data are received, timer3 is disabled, and timer1 is enabled.
    \item This process continues until the STOP command is received. If the command is received while the board is sending data, timer1 is disabled to stop data generation, and the last values (255) are inserted into the ring buffer to stop communication. If received during the PC's data sending session, the remaining data will be received.
\end{enumerate}

\subsubsection{PC Side}
The data receiving process on the PC side mirrors the offline test with shift. After the shift procedure completes, the process continues with reverse transmission.

Initially, after the board's data is generated and shifted, data generation occurs to create data for the PC to send. This process, done as in the \texttt{timer1\_handler} function, involves generating each byte as the previous one plus one, with a maximum value of 253 to ensure 8-bit size. Once 244 bytes or 32,400 bytes are reached, a new BLE packet is sent to the board via Matlab BLE function \texttt{write} in write without response mode. After sending the first data, a timer starts to compute elapsed time during data sending. After all data are sent, it is checked whether the STOP command has been entered. If yes, the program unsubscribes from the characteristic and deletes the callback function. If no, all variables are reset, and the callback function is called to restart the process. This can be summarized by the following state machine:

\begin{itemize}
    \item Device waits for the first byte.
    \item Once received, timer1 is enabled.
    \item After all data are received, timer1 is disabled, throughput is computed, data are shifted, and data generation begins.
    \item When data generation starts, timer2 is enabled, and data are sent to the board.
    \item After completion, timer2 is disabled, and it is checked for the STOP command.
    \item If STOP is received, the process ends; otherwise, it repeats from the beginning.
\end{itemize}
...
For the actual tests, we launched SEGGER in the usual way and ran the code just the same, but instead of having MATLAB open and issuing commands from its interface, we opened two instances of the terminal (Foot): one for the write of the commands, one for the notify.
...
\subsubsection{Our approach to XCDC}

Taking into consideration the fact that XCDC evaluates channels based on same-channel correlation, we wanted to add our own approach that would be able to evaluate also different-channels corrrelation; the intuation behind it being that we also wanted to assess how much channels carry the same information and therefore both reliable and redundant.
Because of this we took the same basic correlation implementation but decided to apply it to different channels.
We hereby list the steps in a more thorough way, we still use "traditional" XCDC for in order to discard poorly evaluated channels.

\begin{itemize}
	\item XCDC between same-channel signals
	\item Score for same-class correlation
	\item Score for different-class correlation
	\item Discard low-score channels
...
\textbf{GAP} \quad The GAP Layer is used to
