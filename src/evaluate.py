...
folders = [f for f in os.listdir(path_arrays + str(animal_num))]
...
for l in range(100):
    ...
    for counts, fil in enumerate(folders):
        ...
        for count, file in enumerate(files):
            ...
            # for i in range(16):
            if not (
                (fil == "touch" and count % 12 != 0)
                or (fil == "prop" and count % 2 != 0)
                or (fil == "prop +" and count % 2 != 0)
            ):
                for j in range(div):
                    datas[j, :, :] = temp[
    ...
    for train_index, val_index in kf.split(x_train):
        ...
        t = copy.copy(x_train_k)
        values = t.reshape(-1, t.shape[-1])
        scaler = StandardScaler()
        scaler = scaler.fit(values)
        x_train_norm = scaler.transform(values).reshape(t.shape)
        ...
        plt.plot(
            history["val_loss"], label="Validation loss", alpha=0.9, color="#5a9aa5"
        )
