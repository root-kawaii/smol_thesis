...
def ENGNet2(nb_classes, Chans, Samples, widths,
            dropoutRate=0.5, kernLength=16, F1=8,
    ...
    input1 = tfkl.Input(shape=(widths, Samples, Chans))

    # input1 = tfkl.Reshape((-1, 4, 1))(input1)
    ##################################################################

    # block1 = tfkl.GlobalAveragePooling2D()(input1)
    # block1 = tfkl.GlobalAveragePooling2D()(input1)
    # block1 = tfkl.Dense(Chans, name='dense2',
    #                     kernel_constraint=tfk.constraints.max_norm(norm_rate), activation='sigmoid')(block1)
    # concat0 = tfkl.multiply([block1, input1])

    ##################################################################

    block1 = tfkl.Conv2D(F1, (1, kernLength), padding='same',
                         input_shape=(Chans, Samples),
                         use_bias=False)(input1)
    block1 = tfkl.BatchNormalization()(block1)
    block1 = tfkl.DepthwiseConv2D((Chans, 1), use_bias=False,
                                  depth_multiplier=D,
                                  depthwise_constraint=tfk.constraints.max_norm(1.))(block1)
    block1 = tfkl.BatchNormalization()(block1)
...
# block2       = SeparableConv2D(F2, (1, 16), use_bias = False, padding = 'same') (block1)
    ...
    block2 = tfkl.SeparableConv2D(F2, (1, Chans),
    ...
    dense = tfkl.Dense(nb_classes, name='dense3',
                       kernel_constraint=tfk.constraints.max_norm(norm_rate))(flatten)
    ...
    model.compile(loss=tfk.losses.CategoricalCrossentropy(),
                  optimizer=tfk.optimizers.Adam(lr=1), metrics=[F1Score()])
