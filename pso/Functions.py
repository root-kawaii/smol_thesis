...
def transform_data_transpose(file_name, file_paths, n_features):
    ...
    for file_number in range(len(file_name)):
        ...
        if file_number == 0:
            x_samp = np.empty(
                (len(file_name), np.shape(sample)[0], n_features))
...
def transform_data(file_name, file_paths, n_features):
    ...
    for file_number in range(len(file_name)):
        sample, file_idx, sample_idx, original_sample, n_samples_original = import_sample_aug(
            file_paths[file_number])
        if (len(np.shape(sample)) == 3):
            # It's dealing with files that Elisa's old code generated, so we have to adequate it
            sample = sample[0, :, 0:16]
        else:
    ...
    if sample.shape[0] > 2500:
        ...
        n_samples_original_vec = np.delete(
            n_samples_original_vec, delete_samples, 0)
...
def generate_data(num_samples, num_channels, num_time_points, num_classes):
    ...
    for i in range(num_classes):
        ...
        class_signals = np.random.randn(
            num_samples, num_channels, num_time_points)
...
def create_model(path, net, n_classes, Samples, kernLength):
    ...
    if (net == 'EEGNet'):
        ...
        model = EEGNet(nb_classes=n_classes,  Samples=Samples,
                       kernLength=kernLength)
        plot_model(model, to_file=name, show_shapes=True, expand_nested=True,
        ...
        model = CNN_temp_elec(sequence_size=(
            Samples, 1), num_classes=n_classes)
        plot_model(model, to_file=name, show_shapes=True, expand_nested=True,
                   layer_range=None, show_layer_activations=True)
        ...
        model = CNN_4x(nb_classes=n_classes,  Samples=Samples,
                       kernLength=kernLength)
        plot_model(model, to_file=name, show_shapes=True, expand_nested=True,
                   layer_range=None, show_layer_activations=True)
        ...
        raise ValueError(
            'toolbox.Functions.create_models non conosce questa rete: ' + net)
...
def create_model_lstm(path, net, n_classes, sequence_size):
    ...
    if (net == 'EEGNet'):
        ...
        model = CNN_temp_elec(sequence_size=sequence_size,
...
def csp(x_samp, y_samp):
    ...
    Wb = np.concatenate((W[0][:, 0:4], W[1][:, 0:4], W[2]
                        [:, 0:4], W[3][:, 0:4]), axis=1)
